...
class SiameseNetwork(nn.Module):
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):
        ...
        tmp=torch.mean(diff[diff.gt(0)])
        return tmp #greater than

    #sentence : (batch_size, max_length)
    def forward(self, anchor, positive=None, negative=None):
        #同时传入两个句子
        if positive is not None and negative is None:
            anchor = self.sentence_encoder(anchor)
            positive = self.sentence_encoder(positive)
            return self.cosine_distance(anchor, positive)
        elif positive is not None and negative is not None:
            anchor = self.sentence_encoder(anchor) #vec:(batch_size, hidden_size)
            positive = self.sentence_encoder(positive)
            negative = self.sentence_encoder(negative)
            return self.loss(anchor, positive, negative,torch.tensor([0.1]))
        else:
