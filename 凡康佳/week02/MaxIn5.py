...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.loss = nn.CrossEntropyLoss(size_average=True, reduce=True)  # 计算交叉熵
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for batch_index in range(train_sample // batch_size): 
            ...
            y = train_y[batch_index * batch_size : (batch_index + 1) * batch_size].flatten()
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        y_p=F.softmax(res, dim=0)
        print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, torch.argmax(y_p), torch.max(y_p)))  # 打印结果
