...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        mask1=torch.ones(x.shape[0],x.shape[1],10)
        mask_1=int(x.shape[1]/2)
        mask_0=x.shape[1]-mask_1
        mask2=torch.zeros(x.shape[0],mask_1,x.shape[1]-10)
        mask4=torch.tril(torch.ones(x.shape[0],mask_0,x.shape[1]-10))
        mask=torch.cat([mask1,torch.cat([mask2,mask4],dim=1)],dim=2)
        mask=mask.cuda()
        # print(x.shape)
        # print(mask.shape)
...
def load_corpus(path):
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            title=json_data['title'][:10]
            content=json_data['content'][:21]
            if len(title)<10:
                title=title+' '*(10-len(title))
            if len(content)<21:
                content=content+' '*(21-len(content))
            corpus.append(title+content) # 多一个错位的时候好凑一个整数
    return corpus

#随机生成一个样本
#从文本中截取随机窗口，前n个字作为输入，最后一个字作为输出
def build_sample(vocab, corpus):
    # start = random.randint(0, len(corpus) - 1 - window_size)
    # end = start + window_size
    window = corpus[:30]
    target = corpus[1:]  #输入输出错开一位
    # print(window, target)
    x = [vocab.get(word, vocab["[UNK]"]) for word in window]   #将字转换成序号
    y = [vocab.get(word, vocab["[UNK]"]) for word in target]
    # # 如果语料长度不足window_size，则用[PAD]补全
    # if len(x) < 30:
    #     x = x + [vocab["[PAD]"]] * (30 - len(x))
    # if len(y) < 30:
    #     y = y + [vocab["[PAD]"]] * (30 - len(y))
    return x, y
...
#window_size 样本长度
#corpus 语料字符串
def build_dataset(batch_size, vocab, corpus):
    dataset_x = []
    dataset_y = []
    for i in range(batch_size):
        choise = random.randint(0, len(corpus) - 1)
        x, y = build_sample(vocab, corpus[choise])
        dataset_x.append(x)
        dataset_y.append(y)
    # print(len(dataset_x))
    # print([len(x) for y in dataset_x])
    # print(len(dataset_y))
    # print([len(y) for y in dataset_y])
...
def train(corpus_path, save_weight=True):
    ...
    for epoch in range(epoch_num):
        ...
        for batch in range(int(train_sample / batch_size)):
            x, y = build_dataset(batch_size, vocab, corpus) #构建一组训练样本
            if torch.cuda.is_available():
                x, y = x.cuda(), y.cuda()
        ...
        print(generate_sentence("广州农业局长回应与王", model, vocab, window_size))
        print(generate_sentence("各路足坛名人陆续抵达", model, vocab, window_size))
    if not save_weight:
        return
    else:
