...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, hidden_size=64):
        ...
        # self.rnn = nn.LSTM(input_size=vector_dim, hidden_size=hidden_size, batch_first=True) # nn.RNN(len(vocab), hidden_size)
        self.rnn = nn.RNN(vector_dim, hidden_size, batch_first=True)
        self.classify = nn.Linear(hidden_size, sentence_length + 1)     #线性层
        # self.activation = torch.sigmoid     #sigmoid归一化函数, 位置预测不用这个激活函数
        # self.loss = nn.functional.mse_loss  #loss函数采用均方差损失, 位置预测不用这个损失函数
    ...
    def forward(self, x, y=None):
        ...
        pooled = torch.mean(rnn_out, dim=1)        # batch_size, sen_len, hidden_size) -> (batch_size, hidden_size)
        ...
        y_pred = self.classify(pooled)                #(batch_size, 1) -> (batch_size, 1)
        if y is not None:
...
def build_vocab():
    ...
    vocab['unk'] = len(vocab)
    print(f'vocab.size={len(vocab)}, vaocab={vocab}')
    return vocab
...
def build_sample(vocab, sentence_length):
    ...
    positions = [ i for i, char in enumerate(x) if char == '你'] # 用你来举例
    if positions:
        y = positions[0] # 取第一个位置
    else:
        y = sentence_length # 表示不存在
    # #指定哪些字出现时为正样本
    # if set("你我他") & set(x):
    #     y = 1
    # #指定字都未出现，则为负样本
...
#建立模型
def build_model(vocab, char_dim, sentence_length, hidden_size=64):
...
def evaluate(model, vocab, sentence_length):
    ...
    with torch.no_grad():
        ...
        # print(f'y_pred={y_pred}, preds={preds}')
...
def predict(model_path, vocab_path, input_strings):
    ...
    with torch.no_grad():  #不计算梯度
        result = model.forward(torch.LongTensor(x))  #模型预测, 这里测试 x 是list, 4个元素, 每个元素对应6条记录 result 是tensor (4, 7)
        softmax_result = torch.softmax(result, dim=1)
        test_preds = torch.argmax(softmax_result, dim=1)

    for i, input_string in enumerate(input_strings):
        pred_pos = test_preds[i].item()
        if pred_pos == sentence_length:
            print(f"输入：{input_string}, 预测：目标字符不存在, softmax_result: {softmax_result[i][pred_pos]:.4f}")
        else:
            print(f"输入：{input_string}, 预测位置：{pred_pos}, softmax_result: {softmax_result[i][pred_pos]:.4f}")
...
if __name__ == "__main__":
    # main()
    test_strings = ["fnvf我e", "wz你dfg", "rqwde你", "n我kww你"]
