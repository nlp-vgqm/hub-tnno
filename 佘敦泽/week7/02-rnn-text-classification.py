...
class RnnClassifier(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=1, dropout=0.1):
        ...
        self.net = nn.RNN(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True)
...
def read_data():
    review_list = []
    labels = []
    with open('./data/text_classification.csv', 'r', encoding='utf-8') as f:
        for line in f.readlines():
            '''
             1,很快，好吃，味道足，量大
            '''
            label, review = line.strip().strip(',')
            review = normal_input(review)
...
def train_rnn_model():
    ...
    net = RnnClassifier(input_size=768, hidden_size=768, output_size=2)
    loss = nn.CrossEntropyLoss()
    optimizer = torch.optim.Adam(params=net.parameters(), lr=lr)

    for epoch in range(epochs):
        data_iter = data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, collate_fn=rnn_collate_fn, drop_last=True)
        for i, (input_ids, token_type_ids, attention_mask, labels) in enumerate(data_iter):
    ...
    torch.save(net.state_dict(), './data/rnn_text_classification.pth')
