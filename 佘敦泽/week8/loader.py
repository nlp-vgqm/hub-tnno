...
class TripleDataGenerator(Dataset):
    def __init__(self, data_path, config):
        self.config = config
        self.path = data_path
        self.vocab = load_vocab(config["vocab_path"])
    ...
    def random_train_sample(self):
        ...
        # 构造三元组负样本情况（较少使用）
            ...
            p, n1, n2 = random.sample(standard_question_index, 3)
            s1 = random.choice(self.knwb[p])
            s2 = random.choice(self.knwb[n1])
            s3 = random.choice(self.knwb[n2])
            return [s1, s2, s3]
...
def load_data_triple(data_path, config, shuffle=True):
    dg = TripleDataGenerator(data_path, config)
    dl = DataLoader(dg, batch_size=config["batch_size"], shuffle=shuffle)
    return dl
