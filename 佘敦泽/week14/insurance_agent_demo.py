...
def get_product_name(product_id: str):
    ...
    """
    product_detail = json.loads(get_product_detail(product_id))
    return product_detail.get("name", "未知产品")
...
请根据用户的问题，使用合适的工具来获取信息并给出专业的建议。"""
    ...
    while iteration < max_iterations:
        ...
        except KeyError as e: # dashscope 没有 tool_calls 属性
            print(e)
            # 没有工具调用，说明模型已经给出最终答案
            print("\n【Agent最终回复】")
            print(response_message.content)
        ...
        for tool_call in tool_calls:
            # function_name = tool_call.function.name # OpenAI
            # function_args = json.loads(tool_call.function.arguments)

            function_name = tool_call['function']['name']  # dashscope
            function_args = json.loads(tool_call['function']['arguments'])
            ...
            if function_name in available_functions:
                ...
                messages.append({
                    "role": "tool",
                    # "tool_call_id": tool_call.id, # OpenAI
...
if __name__ == "__main__":
    ...
    # 示例6：查询产品名称
    run_agent("我想知道产品ID为accident_001的产品名称", model="qwen-plus")
