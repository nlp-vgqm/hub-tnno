...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            if self.config.get("use_triplet", False):
                return self.random_triplet_sample()  # 随机生成一个三元组样本
            else:
                return self.random_train_sample()  # 随机生成一个训练样本
        else:
            return self.data[index]

    # 随机生成三元组样本 (anchor, positive, negative)
    def random_triplet_sample(self):
        ...
        # 随机选择另一个类别作为negative的类别
        negative_classes = [c for c in standard_question_index if c != anchor_class]
        if not negative_classes:
            return self.random_triplet_sample()
        negative_class = random.choice(negative_classes)

        # 从anchor类别中随机选择anchor和positive样本
        anchor, positive = random.sample(self.knwb[anchor_class], 2)

        # 从negative类别中随机选择negative样本
        negative = random.choice(self.knwb[negative_class])

        return [anchor, positive, negative]

    # 依照一定概率生成负样本或正样本（保持原有功能）
