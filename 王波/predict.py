...
"""
模型预测脚本 - 支持LoRA微调模型的加载
"""


class NERPredictor:
    def __init__(self, config, model_path=None, lora_path=None):
        self.config = config
        ...
        print(f"使用LoRA: {config.get('use_lora', False)}")
    ...
    def load_model(self):
        ...
        # 如果是LoRA模型
        if self.config.get("use_lora", False) and self.lora_path:
            print(f"加载LoRA微调模型...")
            ...
            # 非LoRA模型或完整的模型文件
            print(f"加载完整模型权重: {self.model_path}")
            model = TorchModel(self.config)
            model.load_state_dict(torch.load(self.model_path, map_location='cpu'))
        else:
            # 创建新模型
        ...
        print(f"参数压缩比: {(trainable_params / total_params) * 100:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(description="NER模型预测 - 支持LoRA")
    ...
    # 加载配置
    config = Config.copy()  # 创建副本以避免修改原始配置
    ...
    if not args.model_path and not args.lora_path:
        if config.get("use_lora", False):
            ...
            args.model_path = os.path.join(model_dir, f"epoch_{config['epoch']}.pth")
            args.lora_path = os.path.join(model_dir, "lora_weights")
            print(f"使用默认路径: 模型={args.model_path}, LoRA={args.lora_path}")
        else:
            args.model_path = config.get("ori_model_path", "model_output/epoch_20.pth")
    ...
    if args.lora_path and not os.path.exists(args.lora_path):
        print(f"警告: LoRA路径 {args.lora_path} 不存在")
        args.lora_path = None
