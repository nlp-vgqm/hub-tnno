...
def withdraw_products(product_id: str, purchase_days: int):
    if product_id == "life_001":
        if purchase_days <= 10:
            return json.dumps({"success": product_id+"产品可以退订"}, ensure_ascii=False)
        else:
            return json.dumps({"error": product_id+"产品已订购超过10天，不可以无条件退保，详情请咨询业务员"}, ensure_ascii=False)

    if product_id == "health_001":
        if purchase_days <= 13:
            return json.dumps({"success": product_id + "产品可以退订"}, ensure_ascii=False)
        else:
            return json.dumps({"error": product_id + "产品已订购超过13天，不可以无条件退保，详情请咨询业务员"},
                              ensure_ascii=False)

    if product_id == "accident_001":
        if purchase_days <= 15:
            return json.dumps({"success": product_id + "产品可以退订"}, ensure_ascii=False)
        else:
            return json.dumps({"error": product_id + "产品已订购超过15天，不可以无条件退保，详情请咨询业务员"},
                              ensure_ascii=False)
...
tools = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "withdraw_products",
            "description": "申请保险退保，需要提供订购天数",
            "parameters": {
                "type": "object",
                "properties": {
                    "product_id": {
                    ...
                    "purchase_days":{
                        "type": "integer",
                        "description": "产品订购天数（天）",
                        "minimum": 0
                    }
                },
                "required": ["product_id", "purchase_days"]
            }
        }
    },
    {
...
if __name__ == "__main__":
    ...
    # 作业：退保
    run_agent("我半个月前订购了安心人寿保险和医疗险，现在想退保可以吗", model="qwen-plus")
