...
class GraphQA:
    def __init__(self):
        self.graph = Graph("bolt://localhost:7687", auth=("neo4j", "neo4j-demo"))
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("林黛玉的结局是什么")
    print(res)
    res = graph.query("林黛玉和贾宝玉是什么关系")
    print(res)
    res = graph.query("林黛玉的父亲是谁")
    print(res)
    res = graph.query("薛宝钗的判词是什么")
    print(res)
    res = graph.query("贾探春的身份是什么")
    print(res)
    res = graph.query("贾迎春的住所")
    print(res)
