...
class DialogueSystem:
    ...
    def intent_judge(self, memory):
        ...
        max_score = -1
        hit_node = None
        memory["hit_node_repeat"] = False
        for node in memory["available_nodes"]:
            score = self.calucate_node_score(query, node)
            if score > max_score:
                max_score = score
                hit_node = node
        if "node0" in hit_node:
            memory["hit_node_repeat"] = True
        else:
    ...
    def nlg(self, memory):
        ...
        if memory["policy"] == "reply":
            ...
            hit_node = memory["hit_node"]
            node_info = self.all_node_info[hit_node]
            memory["response"] = "好的，我再说一遍。" + self.fill_in_slot(node_info["response"], memory)
        else:
            #policy == "request"
