...
规律：用交叉熵实现5分类任务，5维随机向量最大的数字在哪维就属于哪一类，构造样本如：
x[1,3,2,5,4]，y[0,0,0,1,0]
x[0,3,6,5,4]，y[0,0,1,0,0]
"""
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        index_p = torch.argmax(y_pred,dim=1)
        index_t = torch.argmax(y,dim=1)
        comparison_result = (index_p == index_t)
        equal_count = torch.sum(comparison_result).item()
        print(equal_count)
    print("正确预测个数：%d, 正确率：%f" % (equal_count, equal_count / test_sample_num))
    return equal_count / test_sample_num
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        # print(res)
        index_vec = np.argmax(vec)
        index_res = np.argmax(res)
        print("输入：%s, 真实类别：%d, 预测类别：%d, 概率值：%f" % (vec, index_vec, index_res, res[index_res]))  # 打印结果
