...
def evaluate(model):
    ...
    print("本次预测集中共有%d个0类样本，%d个1类样本，%d个2类样本，%d个3类样本，%d个4类样本" % (
        sum(y == 0), sum(y == 1), sum(y == 2), sum(y == 3), sum(y == 4)))
    correct, wrong = 0, 0
    with torch.no_grad():
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        pos = torch.argmax(res).item()
        print("输入：%s, 预测概率向量：%s, 预测类别：%d, 概率值：%f" % (vec, res, pos, res[pos]))  # 打印结果
