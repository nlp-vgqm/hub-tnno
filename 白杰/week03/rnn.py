...
# -------------------------- 1. 配置参数（确保所有地方使用统一的序列长度） --------------------------
TARGET_CHARS = {"你", "我"}  # 要定位的特定字符
vector_dim = 20              # 字符嵌入维度
sentence_length = 6          # 文本固定长度（关键：训练和预测必须保持一致）
hidden_size = 32             # RNN隐藏层维度
num_layers = 1               # RNN层数
batch_size = 20              # 批次大小
epoch_num = 15               # 训练轮数
train_sample = 800           # 每轮训练总样本数
...
if __name__ == "__main__":
    main()
    # 测试预测
    test_strings = ["你好abc", "我是xyz", "他在123", "a你b我cd", "超长文本示例123456789"]
    predict("rnn_position_model.pth", "vocab_position.json", test_strings)
