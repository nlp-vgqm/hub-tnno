...
class TorchModel(nn.Module):
    def __init__(self, vocab, vector_dim, hidden_size, sentence_length):
        super(TorchModel, self).__init__()
        self.embedding = nn.Embedding(len(vocab), vector_dim)  # embedding层
        self.rnn = nn.RNN(vector_dim, hidden_size, batch_first=True)
        self.layer = nn.Linear(hidden_size, sentence_length + 1)
...
def build_model(vocab, char_dim, hidden_size, sentence_length):
    model = TorchModel(vocab, char_dim, hidden_size, sentence_length)
    return model
...
if __name__ == "__main__":
    main()
    test_strings = ["fnvf我e", "wz你dfg", "rq他deg", "n我kwww"]
